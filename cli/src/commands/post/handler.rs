...
pub async fn handle_post_command(
    ...
    match command {
        PostCommand::Get { id } => {
            ...
            let response: models::post::Post =
                serde_json::from_str(&reqwest::get(request_url).await?.text().await?)?;
            println!("Response: {:#?}", response);
        }
        PostCommand::GetAll => {
            let request_url = format!("{}/posts", _app_config.api_url);
            let response: Vec<models::post::Post> =
                serde_json::from_str(&reqwest::get(request_url).await?.text().await?)?;
            println!("Response: {:#?}", response);
        }
        PostCommand::Create(args) => {
            ...
            let token = _app_config
                .token
                .ok_or(anyhow!("Could not retrieve user token"))?;
